...
import os
from playsound import playsound as ps, playsound
...
attachment_dir = 'C:\\Users\\Manohar\\Desktop\\savings'
...
#             texttospeech(
#                 "Congratulations. You have logged in successfully. You will now be redirected to the menu page.",
#                 file + i)
#             i = i + str(1)
...
#     return render(request, 'Email/login.html', {'detail': detail})
    ...
    if request.method == 'POST':
        ...
        while flag:
            ...
            if addr != 'N':
                ...
                say = speechtotext(10)
                print(say)
                print("Tesrt ", say.lower())
                if say.lower() == 'continue':
                    print(say)
                    flag = False
                    print(say)
                    print("checkkkk",say)
        ...
        while flag:
            ...
            texttospeech("You meant " + passwrd + " say continue to confirm or no to enter again", file + i)
            say = speechtotext(5)
            if say.lower() == "continue":
                flag = False
                print(say)
...
def options_view(request):
    ...
    if request.method == 'POST':
        ...
        while flag:
            texttospeech(
                "To compose an email say compose. To open Inbox folder say Inbox. To open Sent folder say Sent. To "
                "open Trash folder say Trash. To Logout say Logout. Do you want me to repeat?",
                file + i)
            # i = i + str(1)
            say = speechtotext(5)
            print(say)
        ...
        if act == 'compose':
            ...
            addr = ""
            passwrd = ""
            texttospeech("You have been logged out of your account and now will be redirected back to the login page.",
                         file + i)
            i = i + str(1)
        ...
        return render(request, 'Email/options.html')
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            texttospeech("Your email has been sent successfully. You will now be redirected to the menu page.",
                         file + i)
            i = i + str(1)
        except Exception:
            texttospeech(
                r"Sorry, your email failed to send. please try again. You will now be redirected to the the compose "
                r"page again.",
                file + i)
            i = i + str(1)
            return JsonResponse({'result': 'failure'})
    ...
    return render(request, 'Email/compose.html', {'compose': compose})
...
def read_mails(mail_list, folder):
    ...
    for item in mail_list:
        ...
        texttospeech("Email number " + str(
            mail_count + 1) + "    .The mail is from " + From + " to " + To + "  . The subject of the mail is " + Subject,
                     file + i)
        i = i + str(1)
...
def search_specific_mail(folder, key, value, foldername: str):
    global i, conn
    conn.select(folder)
...
def inbox_view(request):
    ...
    if request.method == 'POST':
        ...
        text = "You have reached your inbox. There are " + str(
            ...
            no) + " unread emails" + (". To read unread emails say unread. To search a specific email say search. To "
                                      "go back to the menu page say back. To logout say logout.")
        ...
        while flag:
            ...
            if act == 'unread':
                ...
                while True:
                    ...
                    i = i + str(1)
                    email_id = speechtotext(15)
                    texttospeech("You meant " + email_id + " say yes to confirm or no to enter again", file + i)
                    i = i + str(1)
                    yn = speechtotext(5)
                    yn = yn.lower()
                    if yn == 'yes':
            ...
            texttospeech("If you wish to do anything else in the inbox or logout of your mail say yes or else say no.",
                         file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
...
def sent_view(request):
    ...
    if request.method == 'POST':
        ...
        result1, data1 = conn.search(None, "ALL")
        mail_list = data1[0].split()
        text = "You have reached your sent mails folder. You have " + str(
            len(mail_list)) + ("mails in your sent mails folder. To search a specific email say search. To go back to "
                               "the menu page say back. To logout say logout.")
        texttospeech(text, file + i)
        i = i + str(1)
        flag = True
        while flag:
            ...
            if act == 'search':
                ...
                email_id = email_id.lower()
                email_id = convert_special_char(email_id)
                search_specific_mail('"[Gmail]/Sent Mail"', 'TO', email_id, 'sent')
            ...
            texttospeech(
                "If you wish to do anything else in the sent mails folder or logout of your mail say yes or else say "
                "no.",
                file + i)
            i = i + str(1)
            ans = speechtotext(3)
...
def trash_view(request):
    ...
    if request.method == 'POST':
        ...
        result1, data1 = conn.search(None, "ALL")
        mail_list = data1[0].split()
        text = "You have reached your trash folder. You have " + str(
            len(mail_list)) + ("mails in your trash folder. To search a specific email say search. To go back to the "
                               "menu page say back. To logout say logout.")
        texttospeech(text, file + i)
        i = i + str(1)
        flag = True
        while flag:
        ...
        return render(request, 'Email/trash.html')
